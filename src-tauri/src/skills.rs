...
fn read_tagline(skill_dir: &Path) -> String {
    ...
    let Ok(content) = fs::read_to_string(&skill_md) else { return String::new(); };
    ...
    for line in content.lines() {
        let t = line.trim();
        if t.is_empty() || t.starts_with('#') || t == "---" { continue; }
        if t.starts_with("name:") || t.starts_with("description:") { continue; }
        return t.to_string();
    }
    String::new()
}
...
fn relative_time(modified: SystemTime) -> String {
    ...
    else if secs < 86_400 * 7 { format!("{} days ago", secs / 86_400) }
    else if secs < 86_400 * 30 { format!("{} weeks ago", secs / 86_400 / 7) }
    else { dt.format("%Y-%m-%d").to_string() }
}

/// Which platforms currently have a symlink for this skill pointing back to central.
/// "central" is always included if the skill exists in central.
pub fn compute_routes(skill_id: &str) -> Vec<String> {
    let mut routes = vec!["central".to_string()];
    let Some(central) = central_dir() else { return routes; };
    let source = central.join(skill_id);

    for p in PLATFORMS.iter().filter(|p| !p.is_hub) {
        let Some(platform_root) = expand_path(&p.path) else { continue };
        let target = platform_root.join(skill_id);
        let Ok(meta) = fs::symlink_metadata(&target) else { continue };
        if !meta.file_type().is_symlink() { continue; }
        let Ok(link) = fs::read_link(&target) else { continue };
        // read_link may give a relative path on some systems; canonicalize-compare both.
        let resolved = if link.is_absolute() { link.clone() } else { target.parent().map(|d| d.join(&link)).unwrap_or(link) };
        if resolved == source {
...
pub fn import_skill(name: String, tagline: String) -> Result<Skill> {
    ...
    if dir.exists() {
        return Err(AppError::Conflict(format!("'{}' already exists in central", id)));
    }
    fs::create_dir_all(&dir)?;

    let desc_for_md = tagline.replace('"', "'");
    let body_desc = if tagline.is_empty() { "New skill — fill in what it does." } else { &tagline };
    let skill_md = format!(
