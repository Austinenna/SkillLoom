...
- API key 可存到 macOS Keychain，Settings 会显示已保存状态，但不回显密钥内容
...
- AI 摘要的 live API 请求需要用户在 Settings 里配置 provider、endpoint、model 和 API key；API key 不写入配置文件，可用 Test 按钮先验证。
